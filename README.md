...
- A test app shows how this system works. It invokes helper functions/libraries to obtain quaternion data. It prints data as yaw/pitch/roll to the Serial output (monitor).
  As data-ready events are generated by the **IMU**, the test app does not use its own ```loop()``` function for control.  
